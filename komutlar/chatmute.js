...
exports.run = async (client, message, args) => {
  ...
  if(!message.member.roles.get("734735660262490162") && !message.member.hasPermission('ADMINISTRATOR')) return message.channel.send("Bu Komutu Kullanmaya Yetkin Yok!!").then(m => m.delete(5000));
  ...
  var user = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
    ...
    var muterole = message.guild.roles.get("735586915893510164")
  ...
  .setTimestamp()  
    ...
    let sChannel = message.guild.channels.get("735582391841587212")
    if(!sChannel) return
    sChannel.send(embed)
  },1000)
    },  ms(muteTime));
